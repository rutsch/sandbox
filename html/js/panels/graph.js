var objTrendGraph = {
	...
	correctlabeldecimals: function (strLabel, strReferenceLabel) {
		...
		var strReturnValue = formatMoney(parseFloat(strLabel), intReferenceLabelDecimals, ',', '.', '');
	...
	drawgraph: function (objData) {
		...
		if (self.props.axis.ygridlines == null) {
			...
			arrPoints.push({ value: intValueTopLine, label: self.correctlabeldecimals(intValueTopLine + '', (self.props.axis.ymax + ''), true) });

			//% from bottom
			var intValueBottomLine = Math.round((self.props.axis.ymin + intMargin) * intFactor) / intFactor;
			arrPoints.push({ value: intValueBottomLine, label: self.correctlabeldecimals(intValueBottomLine + '', (self.props.axis.ymax + ''), true) });
